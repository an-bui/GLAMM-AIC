...
```{r}
peng_lm1 <- lm(body_mass_g ~ flipper_length_mm, data = penguins)
AIC(peng_lm1)

peng_lm2 <- lm(body_mass_g ~ flipper_length_mm + bill_depth_mm, data = penguins)
AIC(peng_lm2)

peng_lm3 <- lm(body_mass_g ~ flipper_length_mm + sex, data = penguins)
AIC(peng_lm3)

peng_lm4 <- lm(body_mass_g ~ flipper_length_mm + sex + bill_depth_mm, data = penguins)
AIC(peng_lm4)
```

### b. Choosing between models

Between these models, the best is `peng_lm4`, which has the lowest AIC value of `r round(AIC(peng_lm4), 2)`. There's a big jump in AIC values once we consider penguin sex, but what about comparing `peng_lm3` and `peng_lm4`?  

We can calculate the difference between AIC values using
$$
\Delta_i = AIC_i - AIC_{min}
$$
where $AIC_{min}$ is the smallest AIC value being compared (in this case, `r round(AIC(peng_lm4), 2)`), and $AIC_i$ is the AIC value for the model you want to compare (in this case, the AIC value for `peng_lm3` is `r round(AIC(peng_lm3), 2)`). Therefore, the $\Delta_i$ is `r round(AIC(peng_lm3)-AIC(peng_lm4), 2)`.

The difference between these two models is large enough such that we could say that `peng_lm3` is still the best model to describe the data, and not both `peng_lm3` and `peng_lm4`. If you have two models with similar AIC values, people generally use this rule to evaluate how much those models support your data _together_:
```{r echo = FALSE}
support <- tribble(
...
Following these rules, `peng_lm3` is too different (i.e. the $\Delta_i$ value is greater than 10) to have comparable support from the data as `peng_lm4`.
...
```{r}
# calculate deltai
delta1 <- exp(-0.5*(AIC(peng_lm1) - AIC(peng_lm4)))
delta2 <- exp(-0.5*(AIC(peng_lm2) - AIC(peng_lm4)))
delta3 <- exp(-0.5*(AIC(peng_lm3) - AIC(peng_lm4)))
delta4 <- exp(-0.5*(AIC(peng_lm4) - AIC(peng_lm4)))

# sum likelihoods
sum_likes <- sum(c(delta1, delta2, delta3, delta4))
...
weights <- tribble(
  ...
  "peng_lm4", round(delta4, 2), round(weight4, 2)
) %>% 
  gt() %>% 
  tab_style(
    style = cell_text(size = "small"),
...
Because the $\Delta_i$ values are so small, the weights are also small. From this, we can get information that we already knew: `peng_lm4` is the best model, and all the other models are not at all likely to be as good as `peng_lm4`. However, knowing how to calculate weights is _very_ important when you have a few different models with similar AIC values that you want to be able to compare.
...
##### i. fitting one model with all the predictors

"All these effects are probably real, though some will be stronger and some weaker." Then you can test each predictor using LRT. 

```{r}
huge.model <- glm(count ~ site + sample + cover + mined + DOP + DOY + Wtemp, data=Salamanders, family="poisson")
```

```{r}
plot(allEffects(huge.model))
```
